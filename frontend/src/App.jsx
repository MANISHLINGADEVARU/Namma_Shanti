...
function App() {
  ...
  // Keep selectedIncident in sync with real-time updates from Firebase
  React.useEffect(() => {
    if (selectedIncident) {
  ...
  const handleMassBroadcast = async () => {
    ...
    if (isFirebaseConfigured && selectedIncident.id && !selectedIncident.id.startsWith("inc_")) {
      try {
        ...
        setSelectedIncident(prev => ({ ...prev, status: 'approved', mass_broadcast: true, mass_broadcasted: false }));
      } catch (err) {
